...
if(isset($_GET["code"]))
{
	...
	$exam->query = "
	SELECT user_table.user_id,user_table.user_email_address,user_table.user_gender, user_table.user_image, user_table.user_name
	FROM user_table 
	WHERE user_table.user_id = '".$userID."'";

	$result = $exam->query_result();

	//echo $result[0]["user_image"];
	//return 1;
	...
	//{
		...
		<tr>
			...
			<td>'.$result[0]["user_email_address"].'</td>
			<td>'.$result[0]["user_gender"].'</td>
		</tr>
		';
	...
	AND user_exam_question_answer.user_id = '".$userID."'
	";
